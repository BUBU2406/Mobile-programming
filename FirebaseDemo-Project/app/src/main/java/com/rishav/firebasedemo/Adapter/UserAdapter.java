...
public class UserAdapter extends RecyclerView.Adapter<UserAdapter.ViewHolder>{
    ...
    private List<User> mUsers;// list user đc search
    private boolean isFargment;

    private FirebaseUser firebaseUser;

    public UserAdapter(Context mContext, List<User> mUsers, boolean isFargment) {
        this.mContext = mContext; // chứa tt của dữ liệu
        this.mUsers = mUsers; // list user
        this.isFargment = isFargment;// dùng để hiển thị tt chi tiết của người dùng
    }
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new UserAdapter.ViewHolder(view);// khởi tạo xem tt tại vị trí đó
    }
        ...
        holder.btnFollow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (holder.btnFollow.getText().toString().equals(("follow"))){
                    // realtimeDB tạo 1 lớp con Follow -> từ uid của người đc follow -> following -> setvalue của id người follow
                    FirebaseDatabase.getInstance().getReference().child("Follow").
                            child((firebaseUser.getUid())).child("following").child(user.getId()).setValue(true);
                    // đồng thời trong cái follow -> tạo id của người follow -> followers -> setvalue của người đc follow
                    FirebaseDatabase.getInstance().getReference().child("Follow").
                            child(user.getId()).child("followers").child(firebaseUser.getUid()).setValue(true);
                    // người được follow đó sẽ nhận thông báo
